...
description: |
  De høye strømprisene har de siste ukene vært på alle lepper, avisforsider og sakskart. I hvert fall om en skal tro oppstusset i media. Det er en god anledning til å se på forbruket i det nye huset vårt, og få litt grep om hvor høyt og dyrt strømforbruket er.
author:
...
```{r, echo = FALSE}
#teller opp gjennomsnitt pr dag
temp = group_by(df, dato) %>%
  summarise(forbruk_pr_dag = sum(forbruk))

summary(temp$forbruk_pr_dag)
...
temp = filter(df, natt == "dag") %>%
  group_by(dato) %>%
  summarise(forbruk_prdag = sum(forbruk))
